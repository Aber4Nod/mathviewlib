...
protected:
  ...
  {
      std::cout << "[GlyphStringArea]: this " << this << std::endl;
       assert(children.size() == counters.size()); 
       for (const auto & elem : children)
       {
           SmartPtr<const HorizontalArrayArea> harea = smart_cast<const HorizontalArrayArea>(elem);
           std::cout << "[GlyphStringArea]: child " << harea << std::endl;
           if (harea != nullptr)
               harea->setParent(this);
       }
  }
  virtual ~GlyphStringArea() { }

public:
  static SmartPtr<GlyphStringArea> create(const std::vector<AreaRef>& children, const std::vector<CharIndex>& c, const UCS4String& s) // this is current issue
