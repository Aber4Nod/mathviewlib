...
AreaRef
Qt_Shaper::getGlyphArea(unsigned glyph, const scaled& sp_size) const    // from stringnode format -> shape 
{
    qreal size = Qt_RenderingContext::toQtPixels(sp_size);
    qDebug() << "[getGlyphArea] creating glyph area with glyph: " << glyph;
    Qt_GlyphArea *area = Qt_GlyphArea::create(glyph, m_rawFont, size);
    qDebug() << "[getGlyphArea]: " << area;
    return area;
}
