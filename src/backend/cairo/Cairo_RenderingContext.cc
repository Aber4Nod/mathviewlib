...
Cairo_RenderingContext::fill(const scaled& x, const scaled& y, const BoundingBox& box) const
{
  ...
  std::cout << "[Cairo_RenderingContext]::draw // drawing with coords x = "
        << x.toDouble() << " y = " << -(y + box.height).toDouble()
        << " width = " << box.width.toDouble() << " height = " << (box.height + box.depth).toDouble()
        << std::endl;
                             ...
                             unsigned glyph) const
{
  ...
  std::cout << "[Cairo_RenderingContext]::draw // drawing with coords x = " << x.toDouble() << " y = " << y.toDouble() << std::endl; 
  cairo_set_source_rgba(m_cr, fg.red / 255., fg.green / 255., fg.blue / 255., fg.alpha / 255.);
